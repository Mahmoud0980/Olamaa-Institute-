...
export default function AddContractModal(props) {
  ...
  return <AddContractModalContent {...props} />;
}
...
function AddContractModalContent({ onClose, student, onSaved }) {
  ...
  const [currency, setCurrency] = useState(
    isEdit &&
      existingContract?.final_amount_syp > 0 &&
      !existingContract?.total_amount_usd
      ? "syp"
      : "usd",
  );
  ...
  const [form, setForm] = useState(() => {
    if (isEdit && existingContract) {
      ...
      return {
        total_amount_usd: isSyp ? "" : c.total_amount_usd || "",
        total_amount_syp: isSyp
          ? c.total_amount_usd * c.exchange_rate_at_enrollment
          : "",
  ...
  const computed = useMemo(() => {
    ...
    const finalAmountLocalRaw = Math.max(baseAmountLocal - discountAmountLocal, 0);
            ...
            <div className="space-y-4">
              ...
              {mode === "manual" && installments.length > 0 && (
                <div className="space-y-2">
                  <div className="grid grid-cols-3 gap-2 px-1 text-xs text-gray-500">
                    <div>رقم الدفعة</div>
